...
<body>
  ...
  <script type="text/javascript">
  	...
  	async function getBoards(){  //---------------------------------- 2. 본문에 await 키워드가 포함되기 위해서는 반드시 함수가 async 함수여야 합니다.
  	  const response = await fetch("${contextPath}/b/list");  //----- 1. fetch() 함수의 반환값은 프로미스이므로, await을 추가하여 프로미스에 저장된 값을 꺼냅니다. 
  	...
  	function getJson3(){
  	  ...
  	  fetch("${contextPath}/c/detail?bid=" + bid)
  	  	.then(response => {
  	  	  ...
  	  	  // 404 이외 예외 처리
  	  	  if (!response.ok) {  //----- status가 200(200번대)이면 true
