...
@RequiredArgsConstructor //----- Spring Container에 있는 BoardService 타입의 빈을
                         //      private final BoardService boardService에 자동 주입(@Autowired)하기 위한
...
public class BoardController {
  ...
  @PostMapping("/write")
  public String write(HttpSession session  //--- 세션을 사용하고 싶은 경우 컨트롤러 메소드의 매개변수로 선언
  ...
  @GetMapping("/detail")
  public String detail(int bid  //---------- bid에 해당하는 board 정보를 가져와서
                     , Model model) { //----- Model에 저장한 후 board 폴더 아래 detail.jsp로 forward
  ...
  public String modify(BoardDTO board  //----- title, content, bid를 객체 board가 저장합니다.
    ...
    //----- 전달할 메시지 저장
    redirectAttr.addFlashAttribute("msg", result ? "수정 성공" : "수정 실패")  //- Flash Attribute : redirect 하는 곳으로 보내는 데이터를 저장할 때 사용합니다.
