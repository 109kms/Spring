 ...
 * @Configuration
 * 
 * 1. 클래스 레벨 어노테이션으로, 해당 클래스가 스프링 빈 설정 소스임을 나타냅니다. (나는 Spring Bean Configuration File입니다.)
 * 2. @Bean 어노테이션이 추가된 메소드들이 빈을 생성하는 팩토리 역할을 수행합니다.
 * 3. 내부적으로는 싱글톤 보장이 됩니다.
...
public class AppConfig {
  ...
  Calculator calc() {
    ...
    calculator.setPrice(12345);
    calculator.setAdder(add());  //----- 다른 빈(Bean) 메소드를 호출해서 빈(Bean) 사이 의존성 주입이 가능합니다.
